                ...
                if ($i <= 1) {
                    ...
                    if ($value[0] == 'boolean') {
                        ...
                        if ($isMultiLang) {
                            if(str_contains($key , '[]')){
                                $key = str_replace('[]' ,'' , $key);
                                $out .= "\t\t\t\t\t" . '<a href="{{ url("' . strtolower($this->getNameInput()) . '/".$d->id."/view") }}" ><p>{{ json_decode($d->' . $key . ') ? \Illuminate\Support\Str::limit(implode("," , json_decode($d->' . $key . ')) , 20) : ""}}</a></p > ' . "\n";
                            }else{
                                $out .= "\t\t\t\t\t" . '<a href="{{ url("' . strtolower($this->getNameInput()) . '/".$d->id."/view") }}" ><p>{{ \Illuminate\Support\Str::limit($d->' . $key . '_lang , 20) }}</a></p > ' . "\n";
                            }
                        } else {
                            if(str_contains($key , '[]')){
                                $key = str_replace('[]' ,'' , $key);
                                $out .= "\t\t\t\t\t" . '<p><a href="{{ url("' . strtolower($this->getNameInput()) . '/".$d->id."/view") }}">{{ json_decode($d->' . $key . ') ? \Illuminate\Support\Str::limit(implode("," , json_decode($d->' . $key . ')) , 20) : "" }}</a></p > ' . "\n";
                            }else{
                                $out .= "\t\t\t\t\t" . '<p><a href="{{ url("' . strtolower($this->getNameInput()) . '/".$d->id."/view") }}">{{ \Illuminate\Support\Str::limit($d->' . $key . ' , 20) }}</a></p > ' . "\n";
                            }
                        }
                    }
                }
                ...
                if ($i <= 3) {
                    ...
                    if ($value[0] == 'boolean') {
                        ...
                        if ($isMultiLang) {
                            if(str_contains($key , '[]')){
                                $key = str_replace('[]' ,'' , $key);
                                $out .= "\t\t\t\t\t" . $start . '{{ json_decode($d->' . $key . ') ? \Illuminate\Support\Str::limit(implode("," , json_decode($d->' . $key . ')) , ' . $limit . ') : "" }}' . $end . "\n";
                            }else{
                                $out .= "\t\t\t\t\t" . $start . '{{ \Illuminate\Support\Str::limit($d->' . $key . '_lang , ' . $limit . ') }}' . $end . "\n";
                            }
                            ...
                            if(str_contains($key , '[]')){
                                ...
                                $out .= "\t\t\t\t\t" . $start . '{{ \Illuminate\Support\Str::limit($d->' . $key . ' , ' . $limit . ') }}' . $end . "\n";
                            }
                        }
                    }
                }
            ...
            foreach ($this->cols as $key => $value) {
                ...
                if($count < env('TABLE_COLS')){
                    ...
                    if ($value[0] == 'boolean') {
                        ...
                        if ($isMultiLang) {
                            $out .= '<td>{{\Illuminate\Support\Str::limit($d->' . $key . '_lang , 20) }}</td> ' . "\n\t\t\t\t";
                        } else {
                            $out .= '<td>{{ \Illuminate\Support\Str::limit($d->' . $key . ' , 20) }}</td> ' . "\n\t\t\t\t";
                        }
                    }
                }
            }
