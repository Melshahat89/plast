...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('products.products') , 'model' => 'products' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('admin/products/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.products.relation.categories.edit")
   ...
   <label for="description">{{ trans("products.description")}}</label>
    <textarea name="description" class="form-control tinymce" id="description"   placeholder="{{ trans("products.description")}}" >{{isset($item->description) ? $item->description : old("description") }}</textarea >
   ...
   @endif
              <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('products.products') }}
                </button>
            </div>
        </form>
    @endcomponent
